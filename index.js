...
console.log(findElementGreaterThan([1, 2, 3, 4, 5], 6)); // Виведе 4
...
function getPositiveArrayValues(arr) {
  ...
  return positiveArrayValues.filter((valuePositive) => valuePositive > 0);
...
function sumAndPrint(arr) {
  ...
  const sum = arr.reduce((num, currentNum) => (num += currentNum));
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  const sum = newArr.reduceRight((accum, currNum) => (accum += currNum));
...
function wordLengths(arr) {
  ...
  return arr.map((world) => world.length);
...
function findLongestWord(arr) {
  ...
  const longestWord = arr.reduce((world, currentWorld) => {
    return world.length > currentWorld.length ? world : currentWorld;
  });
...
function calculateTotalPrice(arr) {
  ...
  const totalPrice = arr.reduce((number, currentNum) => {
    return (number += currentNum.price);
  }, 0);
